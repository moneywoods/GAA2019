...
Shader "Hidden/Underwater/Terrain-AddPass"
{
	...
	CGINCLUDE
		...
		half4 LightingCustomLambert(SurfaceOutputCustom s, half3 lightDir, half atten)
		{
			...
			fixed4 caustics0 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z + (anim * _CausticsSpeed), _CausticsCoord.w));
			fixed4 caustics1 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z - (anim * _CausticsSpeed), _CausticsCoord.w));
			fixed4 caustics2 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z, _CausticsCoord.w + (anim * _CausticsSpeed)));
			fixed4 caustics3 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z, _CausticsCoord.w - (anim * _CausticsSpeed)));
