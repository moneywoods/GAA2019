Shader "Underwater/Transparent/Standard"
{
	...
	SubShader
	{
		...
		#pragma surface surf CustomStandard fullforwardshadows nofog finalcolor:applyfog alpha:fade vertex:vert
		...
		float3 getCaustics(float3 worldPosition, fixed anim)
		{
			float4 caustics0 = tex2D(_Caustics, worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z + (anim * _CausticsSpeed), _CausticsCoord.w));
			float4 caustics1 = tex2D(_Caustics, worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z - (anim * _CausticsSpeed), _CausticsCoord.w));
			float4 caustics2 = tex2D(_Caustics, worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z, _CausticsCoord.w + (anim * _CausticsSpeed)));
			float4 caustics3 = tex2D(_Caustics, worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z, _CausticsCoord.w - (anim * _CausticsSpeed)));
			float3 caustics = saturate(caustics0.r + caustics1.g + caustics2.b + caustics3.a);

			return caustics;
		}

		float getCausticsIntensity(float3 worldPosition, float3 worldNorm)
		{
			float causticsIntensity = clamp((worldPosition.y - _CausticsPosition0) / (_CausticsPosition1 - _CausticsPosition0), 0.0f, 1.0f);
			causticsIntensity = lerp(_CausticsIntensity0, _CausticsIntensity1, causticsIntensity);
			causticsIntensity = causticsIntensity * min(1.0f, max(0.0f, dot(worldNorm, float3(0, 1, 0)) + 0.5f));
		...
		inline half4 LightingCustomStandard(SurfaceOutputStandardCustom s, half3 viewDir, UnityGI gi)
		{
			...
			// Caustics
			float4 caustics0 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z + (s.anim * _CausticsSpeed), _CausticsCoord.w));
			float4 caustics1 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z - (s.anim * _CausticsSpeed), _CausticsCoord.w));
			float4 caustics2 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z, _CausticsCoord.w + (s.anim * _CausticsSpeed)));
			float4 caustics3 = tex2D(_Caustics, s.worldPosition.zx * _CausticsCoord.xy + float2(_CausticsCoord.z, _CausticsCoord.w - (s.anim * _CausticsSpeed)));
			float3 caustics = ((saturate(caustics0.r + caustics1.g + caustics2.b + caustics3.a) * (_CausticsBoost + 1)) + _CausticsBoost);

			float causticsIntensity = getCausticsIntensity(s.worldPosition, s.worldNorm);
		...
		void surf (Input IN, inout SurfaceOutputStandardCustom o)
		{
			...
			o.worldNorm = WorldNormalVector(IN, o.Normal);
			o.worldPosition = IN.worldPos;

			float3 caustics = getCaustics(o.worldPosition, o.anim);
			float causticsIntensity = getCausticsIntensity(o.worldPosition, o.worldNorm);
