...
public class TakoController : StateContex
{
    ...
    protected void Awake()
    {
        ...
        SetCurrentState(StateList.Find();
    }
    ...
    public class TakoState : State<StateIndex>
    {
        ...
        public StateIndex CurrentStateIndex { get; protected set; }
    }
    ...
    public class StateNormal : TakoState
    {
        ...
        {
            CurrentStateIndex = StateIndex.Normal;
        ...
        void UpdateByCommand()
        {
            ...
            if(Input.GetKeyDown(KeyCode.Alpha3) || rsh)
            {
                ...
                // takoScript.TransitState()
                // takoScript.SetState(StateIndex.WaitingForKineticPowerEnd);
            }
    ...
    public class StateWaitingForKineticPowerEnd : TakoState
    {
        ...
        {
            CurrentStateIndex = StateIndex.WaitingForKineticPowerEnd;
        }

        void CheckMovingLand()
        {
            // neighvorListに移動中のLandがあるか確認.
            var foundItem = takoScript.neighvorList.Find(item => item.GetComponent<LandStarController>().CheckFlag(LandStarController.LANDSTAR_STAT.MOVING) == true);
            if(foundItem == null)
            { // 無ければステートをNormalへ.
                Context.TransitState(new StateNormal(Context,tako));
            }
        }
    }
    public class StateDisableCommand : TakoState
    {
        public StateDisableCommand(StateContex contex, GameObject tako) : base(contex, tako)
        {
            CurrentStateIndex = StateIndex.CommandDisable;
        }
    }
}